...
const navItems: { href: string; label: string; icon: string; badge?: boolean }[] = [
  { href: "/admin", label: "Dashboard", icon: "dashboard" },
  { href: "/admin/visitors", label: "Visitors", icon: "visitors" },
  { href: "/admin/submissions", label: "Submissions", icon: "submissions", badge: true },
...
type AdminNavProps = { unreadSubmissions?: number };

export function AdminNav({ unreadSubmissions = 0 }: AdminNavProps) {
  ...
  return (
    <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
      {navItems.map((item) => {
        ...
        const showBadge = item.badge && unreadSubmissions > 0;
        return (
          <Link
            key={item.href}
            href={item.href}
          ...
          >
            ...
            {showBadge && (
              <span className="min-w-[1.25rem] rounded-full bg-amber-500 px-1.5 py-0.5 text-xs font-semibold text-white">
